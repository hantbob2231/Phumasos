...
let config={searchTemplate:'',allowedOrigins:new Set()};
...
export async function initBrowser(){
  try{const r=await fetch('/api/config',{credentials:'same-origin'});if(r.ok){const j=await r.json();config.searchTemplate=j.browserSearchTemplate||'';config.allowedOrigins=new Set(Array.isArray(j.proxyAllowedOrigins)?j.proxyAllowedOrigins:[]);$('#proxy-status').textContent=j.proxyEnabled?'Proxy + Direct':'Direct mode';}}
...
function shouldUseProxy(url){
  try{return config.allowedOrigins.has(new URL(url).origin);}catch{return false;}
}
function showCompatibilityError(tab,target,reason='This website could not be displayed inside FigureOS.'){
  const panel=panels.get(tab.id);if(!panel)return;
  panel.innerHTML=`<div class="browser-start browser-compat"><div class="start-date">FIGURE BROWSER</div><h2 style="font-size:32px;margin:0 0 12px">Website unavailable here</h2><p style="max-width:640px;color:var(--muted);line-height:1.6">${escapeHtml(reason)}</p><p style="max-width:640px;color:var(--muted);font-size:13px">Some sites block embedded browsers with their own security settings. FigureOS does not bypass those protections.</p><button class="setting-button" data-open-external>Open normally</button></div>`;
  panel.querySelector('[data-open-external]')?.addEventListener('click',()=>window.open(target,'_blank','noopener,noreferrer'));
...
export function navigate(url,tab=activeTab(),opts={push:true}){
  ...
  const frame=document.createElement('iframe');frame.className='browser-frame';frame.dataset.tabFrame=tab.id;frame.setAttribute('sandbox','allow-scripts allow-forms allow-same-origin allow-popups allow-modals allow-downloads allow-pointer-lock');frame.setAttribute('allow','autoplay; fullscreen; clipboard-read; clipboard-write');const useProxy=shouldUseProxy(target);tab.transport=useProxy?'proxy':'direct';frame.src=useProxy?proxify(target):target;panel.appendChild(frame);tab.frame=frame;
  frame.addEventListener('load',()=>onFrameLoad(tab,frame));
  frame.addEventListener('error',()=>{if(tab.transport==='direct')showCompatibilityError(tab,target);else finishLoading(tab);});
  saveTabs();renderTabs();renderActive();updateToolbar();updateTaskState();
}
function onFrameLoad(tab,frame){
  if(tab.frame!==frame)return;let dest=tab.url;let title=tab.title||new URL(dest).hostname;let fav='';
  try{const href=frame.contentWindow.location.href;dest=unproxify(href)||href||dest;}catch{}
  try{title=frame.contentDocument.title||new URL(dest).hostname;const icon=frame.contentDocument.querySelector('link[rel~="icon"]');fav=icon?.href||(tab.transport==='proxy'?proxify(new URL('/favicon.ico',dest).href):'');frame.contentWindow.addEventListener('keydown',handleShortcut);}catch{title=title==='New Tab'?new URL(dest).hostname:title;}
